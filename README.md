...
- **Layered agent personas.** Each file goes through a `code-reviewer` pass and then a `security-auditor` pass, using pinned, hash-verified persona prompts sourced from Addy Osmani's `agent-skills` repository — see [`docs/adr/0003-pin-and-hash-verify-persona-prompts.md`](docs/adr/0003-pin-and-hash-verify-persona-prompts.md) for why they're pinned.
